...
public class HadronCuts {
    ...
    public static boolean TrajChi2Cut(Float chi2pid) {
        int min_chi2 = -3;
        int max_chi2 = 3;
        return chi2pid > min_chi2 && chi2pid < max_chi2;
    }

    //trajectory crosses  (layers: 6 = DC region 1 start,  18 = DC region 2 center,  36 = DC region 3 end ) 
    public static boolean DCFiducialCutChi2(Integer dc_sector, int region, double x, double y, double z, int partpid) {
        ...
        double theta_DCr = Math.toDegrees(  Math.acos(z / Math.sqrt( Math.pow(x,2) + Math.pow(y,2) + Math.pow(z,2) ) ) );
        double phi_DCr_raw = Math.toDegrees( Math.atan2( y / Math.sqrt( Math.pow(x,2) + Math.pow(y,2) + Math.pow(z,2) ) , x/Math.sqrt( Math.pow(x,2) + Math.pow(y,2) + Math.pow(z,2))));
        
        double phi_DCr = 5000000;
        ...
        switch(partpid){ //event.pid[index]){
            case 2212:
                pid = 1;
                break;
            case 211:
            ...
            case -321:
                pid = 5;
                if( ( region == 3 && dc_sector == 5 ) || dc_sector == 6 ) pid = 4;
                break;
        ...
        dc_sector = dc_sector - 1 ;
        
        
        double a_min = minparams[pid][dc_sector][region-1][0];
        double b_min = minparams[pid][dc_sector][region-1][1];
        double c_min = minparams[pid][dc_sector][region-1][2];
        double d_min = minparams[pid][dc_sector][region-1][3];

        double a_max = maxparams[pid][dc_sector][region-1][0];
        double b_max = maxparams[pid][dc_sector][region-1][1];
        double c_max = maxparams[pid][dc_sector][region-1][2];
        double d_max = maxparams[pid][dc_sector][region-1][3];

        

        double calc_phi_min = a_min + b_min*Math.log(theta_DCr) + c_min*theta_DCr + d_min*theta_DCr*theta_DCr;

        double calc_phi_max = a_max + b_max*Math.log(theta_DCr) + c_max*theta_DCr + d_max*theta_DCr*theta_DCr;
        
        // System.out.println((phi_DCr > calc_phi_min) && (phi_DCr < calc_phi_max));
